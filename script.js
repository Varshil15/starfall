...
preloadResources();
// ========================================
// 🎄 CHRISTMAS EFFECTS 2025 🎄
// ========================================

// Christmas Cursor Trail Effect
let christmasCursorTrail = [];
document.addEventListener('mousemove', (e) => {
    if (christmasCursorTrail.length > 15) {
        const oldTrail = christmasCursorTrail.shift();
    ...
    christmasCursorTrail.push(trail);
});
...
window.addEventListener('load', () => {
    ...
    // Show Christmas popup after 1 second
    setTimeout(() => {
        if (christmasPopup) {
...
// Close popup when clicking Shop Now button
if (shopNowBtn) {
    shopNowBtn.addEventListener('click', closeChristmasPopup);
}
...
document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape' && christmasPopup && christmasPopup.classList.contains('active')) {
        closeChristmasPopup();
    }
});